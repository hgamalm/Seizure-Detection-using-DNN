...
%%%%%% MATLAB CODE FOR EMD DECOMPOSITION TECHNIQUE %%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%emdpar: Emd parabolic decomposition with extrapolated extrema           
%
%   Usage:  rParabEmd= emdpar(x,qResol, qResid, qAlfa);
%           x      - input signal - must be a real vector
%           qResol - Resolution (in DBs: 10*log(WSignal/Bias energy))- normally between 40 and 60 dB 
%           qResid - Residual energy (in DBs: 10*log (WSignal/WqResidual))-
%           normally between 40 and 60 dB
...
function rParabEmd = emdpar (x, qResol, qResid, qAlfa)
dbstop if warning
if(nargin~=4), error('emdpar: Use with 4 inputs.'), end
if(nargout>1), error('emdpar: Use with just one output.'), end
...
while ((qDbResid<qResid) && (quntOscCnt>2) )   % c has some energy and oscilates
    ...
    while true(1)             % inner loop to find each IMF
        ...
        if WBias*WImf<0 , warning('emdpar: Ooops, negative energy detected.'), end
...
[qL, qC] = size(x);
if ((qL*qC)~= max(qL,qC)), error('emdpar: Input signal must be a one dim vector.'), end
if ((qL*qC)<= 1), error('emdpar: Input signal must be a vector.'), end

[qL,qC] = size(qResol);
if ( ~((qL==1)&(qC==1)) ), error('emdpar: Input resolution must be a scalar.'), end
if ( qResol<=0 ), error('emdpar: Input resolution must strictly positive.'), end

[qL,qC] = size(qResid);
if ( ~((qL==1)&(qC==1)) ),  error('emdpar: Input residual must be a scalar.'),  end
if ( qResid<=0 ), error('emdpar: Input residual must strictly positive.'), end

[qL,qC] = size(qAlfa);
if ( ~((qL==1)&(qC==1)) ), error('emdpar: qAlfa step must be a scalar.'), end
if ( qAlfa<=0 ), error('emdpar: qAlfa step  must be strictly positive.'),  end
end
